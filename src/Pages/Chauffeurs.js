...
const Cells = ["email","Nom", "Ville","Tel","Salaire","actions"]
...
function Chauffeur() {
    ...
    function getdata() {
        axios("http://localhost:4000/api/v1/Chauffeur")
        .then((result)=> {
            setData(result.data)
        })
    }
    ...
    function save(){
        axios
            .post(`http://localhost:4000/api/v1/Chauffeur`,{
                    Nom: name,
                    Email: email,
                    Tel: tel,
                    Password: password,
    ...
    function deleteRow() {
        axios
            .delete(`http://localhost:4000/api/v1/Chauffeur/${dataId}`)
            .then(res=>{
                console.log(res);
                setDOpen(false)
                getdata()
    ...
    function getChauffeurById(id,Email,Nom,Tel,Ville,salaire){
        setName(Nom)
        setEmail(Email)
        setTel(Tel)
        setVille(Ville)
    ...
    function updateData(){
        axios
            .put(`http://localhost:4000/api/v1/Chauffeur/${idRow}`,{
                Nom: name,
                Email: email,
                Tel: tel,
                Ville: ville,
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={row.id}>
                                            ...
                                            <TableCell className={classes.Cell}>{row.Salaire}</TableCell>
                                            ...
                                            <EditIcon onClick={() => getChauffeurById(row._id,row.Email,row.Nom,row.Tel, row.Ville, row.Salaire )} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
