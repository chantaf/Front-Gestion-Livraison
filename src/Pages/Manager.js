...
function Manager() {
    ...
    function getdata() {
        axios("http://localhost:4000/api/v1/Manager")
        .then((result)=> {
            setData(result.data)
        })
    }
    ...
    function save(){
        axios
            .post(`http://localhost:4000/api/v1/Manager`,{
                    Nom: name,
                    Email: email,
                    Tel: tel,
                    Password: password,
    ...
    function deleteRow() {
        axios
            .delete(`http://localhost:4000/api/v1/Manager/${dataId}`)
            .then(res=>{
                console.log(res);
                setDOpen(false)
                getdata()
    ...
    function getManagerById(id,Email,Nom,Tel, Ville){
        setName(Nom)
        setEmail(Email)
        setTel(Tel)
        setVille(Ville)
    ...
    function updateData(){
        axios
            .put(`http://localhost:4000/api/v1/Manager/${idRow}`,{
                Nom: name,
                Email: email,
                Tel: tel,
                Ville: ville
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={row.id}>
                                            ...
                                            <EditIcon onClick={() => getManagerById(row._id,row.Email,row.Nom,row.Tel, row.Ville )} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
